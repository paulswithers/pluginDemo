 ...
 * 
 */
public class StarterOpenDomino {
	 ...
	 * </ol>
	 */
	public static class StarterFunction extends GeneratedWrapperObject.Function {
		 ...
		 * @since org.openntf.domino.xsp 2.5.0
		 */
		protected StarterFunction(final JSContext context, final Method crystal) {
		...
		protected MethodMap getMethodMap() {
			return StarterOpenDomino.getMethodMap(null, getWrappedClass());
		}
	 ...
	 * objects at runtime.
	 */
	public static class StarterObject extends GeneratedWrapperObject {
		 ...
		 * @since org.openntf.domino.xsp 2.5.0
		 */
		protected StarterObject(final JSContext paramJSContext, final Object paramObject, final Class<?> clazz) {
		 ...
		 * @since org.openntf.domino.xsp 2.5.0
		 */
		protected StarterObject(final JSContext paramJSContext, final Object paramObject, final String paramString, final Class<?> clazz) {
			super(paramJSContext, paramObject, paramString);
			uiName_ = paramString;
			clazz_ = clazz;
		}
		 ...
		 */
		@Override
		public StarterMethodMap getMethodMap() {
			return StarterOpenDomino.getMethodMap(getJSContext(), getWrappedClass());
		}
	}
	 ...
	 */
	public static class StarterMethodMap extends GeneratedWrapperObject.MethodMap {
		public StarterMethodMap(final JSContext arg0, final Class<?> arg1, final Class<?>[] arg2) {
			super(arg0, arg1, arg2);
		}

	}

	private final static Map<Class<?>, StarterMethodMap> classMap_ = new HashMap<Class<?>, StarterMethodMap>();
	 ...
	 */
	private static Map<Class<?>, StarterMethodMap> getClassMap() {
	 ...
	 * @since org.openntf.domino.xsp 2.5.0
	 */
	private static StarterMethodMap generateMethodMap(final JSContext context, final Class<?> clazz) {
		StarterMethodMap methodMap = new StarterMethodMap(context, clazz, null);
		for (Method crystal : clazz.getMethods()) {
			if (Modifier.isPublic(crystal.getModifiers()) && !Modifier.isStatic(crystal.getModifiers())) {
				...
				if (cur == null) {
					...
					cur = new StarterFunction(context, crystal);
					methodMap.put(crystal.getName(), cur);
				}
				if (cur instanceof StarterFunction) {
					((StarterFunction) cur).addMethod(crystal);
				} else {
					...
					try {
						if ("function:IBMJS built-in function".equals(cur.toString())) {
							methodMap.put(crystal.getName(), new StarterFunction(context, crystal));
						} else {
		...
		synchronized (StarterOpenDomino.class) {
	 ...
	 * @since org.openntf.domino.xsp 2.5.0
	 */
	private static StarterMethodMap getMethodMap(final JSContext context, final Class<?> clazz) {
		StarterMethodMap result = getClassMap().get(clazz);
		if (result == null) {
	 ...
	 * @since org.openntf.domino 2.5.0
	 */
	private static final class StarterConstructor extends GeneratedWrapperObject.EmptyConstructor {
		 ...
		 * @since org.openntf.domino 2.5.0
		 */
		private StarterConstructor(final JSContext context, final Class<?> clazz) {
	 ...
	 * @since org.openntf.domino.xsp 2.5.0
	 */
	private static class StarterWrapperFactory implements IWrapperFactory {
		private final Class<?> clazz_;
		 ...
		 * @since org.openntf.domino.xsp 2.5.0
		 */
		private StarterWrapperFactory(final Class<?> clazz) {
			clazz_ = clazz;
		 ...
		 */
		@Override
		public StarterMethodMap getMethodMap(final JSContext context) {
			return StarterOpenDomino.getMethodMap(context, clazz_);
		}
		 ...
		 */
		@Override
		public StarterObject wrap(final JSContext context, final Object object) {
			return new StarterObject(context, object, clazz_);
		}
	...
	/**
	 * Registers the methods of all Java classes in {@link StarterOpenDomino#WRAPPED_CLASSES}, prefixing them with "Open"
	 * 
	 * @param context
	...
	public static void register(final JSContext context) {
		...
		try {
			...
			registry.registerPackage("StarterDomino", 1337);
			FBSDefaultObject defaultObject = registry.getRegistryObject();

			for (Class<?> clazz : WRAPPED_CLASSES) {
				registry.registerWrapper(clazz, new StarterWrapperFactory(clazz));
				defaultObject.createProperty("Open" + clazz.getSimpleName(), 1338, new StarterConstructor(context, clazz));
			}
	...
	static {
		// e.g. WRAPPED_CLASSES.add(org.openntf.domino.ACL.class);
	}
